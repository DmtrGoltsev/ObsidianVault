...
доказательства:
  - "[[Док_Release_NewDis_20260608]]"
  - "[[Док_Release_Planning_MVP_20260607]]"
  - "[[Finance/01_Человеку/Доказательства/QA_Результаты]]"
...
Практичная карта для дизайнеров Finance: какие пользовательские пути уже видны в продуктовой документации и UI-слое Android/PWA, где смотреть экраны, какие боли и риски упрощать. Карта актуализирована под release `20260607T225457Z-819b5815`, Planning iteration MVP и closure `newDis` commit `6ce31f53f6150050b4cb0dad8488254bd04ff31b`.
...
- Release closure `newDis`: branch `newDis`, commit `6ce31f53f6150050b4cb0dad8488254bd04ff31b`, subject `feat(finance): simplify newDis UX flows`; см. [[Док_Release_NewDis_20260608]].
- Production frontend для `newDis` подтвержден через `/finance/COMMIT` и byte-hash parity `/finance/`, `/finance/sw.js`, manifest, JS/CSS с local `apps/web-pwa/dist`.
- Backend redeploy waiver принят: final HEAD не меняет `apps/backend`, `db`, `api`; backend health `/finance-api/health` 200, но exact backend commit endpoint отсутствует.
- Android evidence для `newDis`: unit XML 60 tests без failures/errors/skipped; lint 0 errors / 6 warnings; APK debug-signed.
...
- Planning в `newDis` — Android-first, пока release evidence не подтвердит отдельный PWA-путь.
- Overview — read-only режим: сохранение возможно только в явное личное или общее.
...
- MVP не включает bank API, Excel/CSV/PDF import pipeline, SMS/push/notification interception, налоговую аналитику, инвестиционные рекомендации и автоматическую переоценку активов. <!-- gitleaks:allow false-positive scope statement -->
...
- Для `newDis` copy считать backend OCR request фактической базой: нельзя обещать Android on-device OCR без отдельной реализации и QA-доказательств.
...
- В release copy указывать, что authenticated production login/OCR smoke и retention/privacy evidence еще должны быть принесены отдельно.
...
- Если команда хочет публично обещать Android on-device OCR, нужны отдельная реализация, QA и security/privacy evidence; до этого copy использует backend OCR request -> pending drafts.
