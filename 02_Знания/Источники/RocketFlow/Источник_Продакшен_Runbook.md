...
Процедуры деплоя бэкенда на HexCore (systemd unit), обновления web-статики (Nginx), проверки состояния БД (PostgreSQL 16). Команды для перезапуска, просмотра логов, отката. Контакты для эскалации.
