...
| `n8nagents-actions` | Активное расширение OpenClaw `0.1.3` |
...
Два защищённых systemd-таймера ежеминутно вызывают внутренние HMAC-подписанные webhook. Ранняя контрольная отправка была доставлена ровно один раз (`message_id=56`) без изменения двух существовавших задач владельца.
...
После проверки Action API и оба свежих планировщика активны; успешные выполнения с 10:00 UTC — `2 / 24 / 24`, выполняющихся — `0`. Оба таймера активны, OpenClaw и n8n работают без перезапусков. Webhook Telegram отсутствует, ожидающих обновлений `0`, обработка `callback_query` разрешена.
