...
доказательства: []
теги: ["openclaw", "n8n", "архитектура", "целевая-модель"]
---

# Архитектура OpenClaw и n8n

Документ описывает целевое устройство системы. До итоговой проверки на рабочем сервере он не заменяет [[CURRENT_STATE_N8NAgents_2026-08-29]].
...
flowchart LR
    ...
    O --> D[DeepSeek]
    O --> M[(Markdown-память)]
    O --> A[Закрытый интерфейс действий]
    A --> N[n8n]
    N --> P[(PostgreSQL)]
    N --> X[Разрешённые внешние службы]
    N --> O
    O --> U
```
