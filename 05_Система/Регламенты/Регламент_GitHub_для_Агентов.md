...
Перед первым push данного агента в любой репозиторий — существующий или новый — обязательно прочитать и выполнить [[Регламент_GitHub_Actions_без_лишних_запусков]], после чего провести trigger inventory и проверки по регламенту. То же требование действует перед любым изменением `.github/workflows/`. Повторное чтение перед каждым push в том же неизменённом контексте не требуется. Неограниченный `on: push` без явного решения по веткам запрещён.
...
- [[Регламент_GitHub_Actions_без_лишних_запусков]] — политика CI-триггеров и уведомлений
