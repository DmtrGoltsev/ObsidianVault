...
Перед первым push в новый репозиторий или изменением `.github/workflows/` выполнить [[Регламент_GitHub_Actions_без_лишних_запусков]]. Неограниченный `on: push` без явного решения по веткам запрещён.
