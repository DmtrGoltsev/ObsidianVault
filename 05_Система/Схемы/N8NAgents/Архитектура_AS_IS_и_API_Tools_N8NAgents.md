...
На 2026-08-30 внедрение OpenClaw находится в работе. До итоговой проверки фактической считается ранее принятая схема, где входящие сообщения Telegram обрабатывает n8n. После успешного переключения этот статус должен быть обновлён.
...
## Целевая схема OpenClaw + n8n

```mermaid
flowchart LR
    U[Владелец в Telegram] --> O[OpenClaw]
    O --> C[Понимание просьбы и сбор контекста]
    C --> T[Выбор одного разрешённого инструмента]
    T --> A[Закрытый Action API]

    subgraph N[Контролируемый контур n8n]
        V[Проверка подписи, отправителя и структуры]
        R[Применение правил]
        H{Нужно подтверждение?}
        P[Ожидание подтверждения человека]
        E[Выполнение операции]
        J[Запись результата и аудита]
        V --> R --> H
        H -->|да| P --> E
        H -->|нет| E
        E --> J
    ...
    A --> V
    J --> S[Структурированный результат]
    S --> O
    O --> U
```
...
На 2026-08-30 этот интерфейс проверен положительными и отрицательными тестами. Его внутренний адрес: `http://n8n:5678/webhook/internal/actions/v1`. Адрес указан для компонентов системы; из интернета он недоступен.
...
## Что считается успешным завершением внедрения

OpenClaw можно признать частью фактической архитектуры только после проверки, что:
...
Если переключение Telegram на OpenClaw не проходит проверку:
