...
- Backend: 21 фикс применён (FIX-A — FIX-U), BE-101 исправлен, FCM модуль добавлен
...
- **Production redeployed (2026-06-12)**: JAR пересобран на сервере (git clone + mvn package), env vars выровнены (DB_USERNAME, SPRING_DATASOURCE_URL), backend UP, API 403 OK через nginx
- **Android URL fix (2026-06-13)**: BASE_URL исправлен с `/api/` на `/focus-api/`, nginx proxy `/focus-api/` → `8082/api/`. E2E на эмуляторе: регистрация 201, автологин, загрузка folders 200 — **подтверждено работает**
