...
Статус: `PASS` на рабочем сервере. Полная внутренняя цепочка включена и проверена. Плагин OpenClaw `0.2.0` загружен, оба процесса Wave 4 активны и имеют активную версию.
...
```mermaid
flowchart LR
    A["Telegram → OpenClaw"] --> B["Плагин 0.2.0: 5 напоминаний + 13 инструментов Poruchik"]
    B -->|"подписанный внутренний запрос"| C["Два активных процесса Wave 4 в n8n"]
    C --> D["Закрытый исполнитель проверки подписи"]
    D --> E["Серверное ядро задач"]
    E --> F["PostgreSQL"]
    E --> G["Обезличенный аудит"]
```
...
## Фактическое состояние после включения

На 1 сентября 2026 года:

- `PoruchikIntentDarkW4` и `PoruchikApprovalDarkW4` активны; у обоих заполнен `activeVersionId` (идентификатор активной версии);
- OpenClaw загрузил плагин `n8nagents-actions` версии `0.2.0`;
- разрешены ровно пять прежних инструментов напоминаний и тринадцать инструментов Poruchik; четырнадцать запрещённых возможностей остаются в списке запретов;
- готовность после перезапуска подтверждена отдельным подписанным запросом списка задач с ответом HTTP `200`;
- все восемь контейнеров находятся в состоянии `healthy`, число перезапусков — `0`;
- Telegram `getMe` завершился успешно;
- исполнитель не публикует порт на узле;
- публичных портов Poruchik нет; мобильный шлюз доступен только через `127.0.0.1` и SSH-туннель;
- тестовые записи, временные HMAC-файлы и сценарии проверки удалены; после очистки подписанная проверка готовности снова дала HTTP `200`.
...
Все тринадцать команд прошли через фактический маршрут установленного плагина. Точный повтор одной операции вернул прежний результат без второй задачи. После перезапуска n8n и исполнителя тот же повтор вернул побайтно идентичный ответ и также не создал дубликат. Использование того же идентификатора операции с изменённым содержимым отклонено кодом `IDEMPOTENCY_KEY_REUSE`.
...
- получение личности только из доверенного серверного контекста и невозможность переопределить её параметрами модели;
- отсутствие содержимого пользовательских задач в техническом аудите: число запрещённых ключей равно `0`.
...
Серверная Wave 4 завершена. Не выполнены только действия, требующие владельца или физических устройств: постоянная подпись APK, установка на телефон, разрешения PIN-кода или биометрии, микрофона и уведомлений, настройка FCM, второй участник и физическая проверка приложения, работы без сети и голоса. Полный список: [[17_Ручной_пакет_завершения_MVP]].

Ограниченный SSH-контур установлен и автоматически проверен; см. [[15_Защищенное_SSH_подключение_рабочего_сервера]]. Текущий единый снимок состояния: [[16_Манифест_текущего_выпуска_2026-09-01]].
